...
import { Home, BarChart3, TrendingUp, MessageSquare, UserCircle, LogOut } from 'lucide-react';

const navItems = [
  { key: 'dashboard', label: 'Dashboard', to: '/analyst/dashboard', icon: Home },
  { key: 'sentiment', label: 'Sentiment Analysis', to: '/analyst/sentiment', icon: BarChart3 },
  { key: 'incident', label: 'Incident Analytics', to: '/analyst/incident', icon: TrendingUp },
  { key: 'social', label: 'Social Media Analytics', to: '/analyst/social', icon: MessageSquare },
  { key: 'profile', label: 'Profile', to: '/analyst/profile', icon: UserCircle },
];
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className={`bg-white shadow-sm transition-all duration-300 flex flex-col ${open ? 'w-64' : 'w-16'} sticky top-0 self-start h-screen overflow-y-auto shrink-0 z-40`}>
        ...
        <div className="p-3 flex items-center gap-3 shadow-inner">
          <img className="w-8 h-8 rounded-full object-cover" src={profile.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name || 'Analyst')}&background=0EA5E9&color=fff`} alt="User" />
          {open && (
            <div className="min-w-0">
              <div className="text-sm font-semibold text-gray-800 truncate">{profile.name || 'Data Analyst'}</div>
              <div className="text-xs text-gray-500 truncate">{profile.email || 'analyst@storm.gov.in'}</div>
            </div>
          )}
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0 overflow-x-hidden">
        <div className="h-16 bg-white shadow-sm sticky top-0 z-30 flex items-center px-4 justify-between">
          <div className="font-semibold text-gray-800">Data Analyst Portal</div>
          <button
